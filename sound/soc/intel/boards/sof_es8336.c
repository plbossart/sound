...
static void pcm_pop_work_events(struct work_struct *work)
{
	...
	gpiod_set_value_cansleep(priv->gpio_speakers,
				 priv->enable_spk_gpio.active_low ? !priv->speaker_en : priv->speaker_en);
	gpiod_set_value_cansleep(priv->gpio_headphone,
				 priv->enable_spk_gpio.active_low ? priv->speaker_en : !priv->speaker_en);
}
	...
	case SNDRV_PCM_TRIGGER_STOP:
		if (priv->speaker_en == true)
			if (substream->stream == 0) {
				...
				gpiod_set_value_cansleep(priv->gpio_speakers,
							 priv->enable_spk_gpio.active_low ? false : true);
				gpiod_set_value_cansleep(priv->gpio_headphone,
							 priv->enable_hp_gpio.active_low ? false : true);
			}
		break;
	default:
		return -EINVAL;
