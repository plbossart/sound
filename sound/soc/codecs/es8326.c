...
static int es8326_probe(struct snd_soc_component *codec)
{
	...
	ret = device_property_read_u8(codec->dev, "amic1-src", &es8326->amic1_src);
	if (ret != 0) {
		dev_dbg(codec->dev, "amic1-src return %d", ret);
		es8326->amic1_src = ES8326_MIC1_SEL;
	}

	ret = device_property_read_u8(codec->dev, "amic2-src", &es8326->amic2_src);
	if (ret != 0) {
		dev_dbg(codec->dev, "amic2-src return %d", ret);
		es8326->amic2_src = ES8326_MIC1_SEL;
	}

	ret = device_property_read_u8(codec->dev, "mic1-src", &es8326->mic1_src);
	if (ret != 0) {
		dev_dbg(codec->dev, "mic1-src return %d", ret);
		es8326->mic1_src = ES8326_ADC_AMIC;
	}
	dev_dbg(codec->dev, "mic1-src %x", es8326->mic1_src);

	ret = device_property_read_u8(codec->dev, "mic2-src", &es8326->mic2_src);
	if (ret != 0) {
		dev_dbg(codec->dev, "mic2-src return %d", ret);
		es8326->mic2_src = ES8326_ADC_DMIC;
	}
	dev_dbg(codec->dev, "mic2-src %x", es8326->mic2_src);

	ret = device_property_read_u8(codec->dev, "jack-pol", &es8326->jack_pol);
	if (ret != 0) {
		dev_dbg(codec->dev, "jack-pol return %d", ret);
